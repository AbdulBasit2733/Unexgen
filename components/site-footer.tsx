import Image from 'next/image'
import type { SVGProps } from 'react'
import { AnimateIn } from '@/components/animate-in'
import logo from '@/public/logo-white-bg.png'
import Link from 'next/link'
function LinkedinIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.13 1.45-2.13 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.46v6.28zM5.34 7.43a2.06 2.06 0 1 1 0-4.13 2.06 2.06 0 0 1 0 4.13zM7.12 20.45H3.55V9h3.57v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.22.79 24 1.77 24h20.45c.98 0 1.78-.78 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z" />
    </svg>
  )
}

function FacebookIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07c0 6.02 4.39 11.01 10.13 11.93v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.69.24 2.69.24v2.97h-1.52c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8V24C19.61 23.08 24 18.09 24 12.07z" />
    </svg>
  )
}

function InstagramIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41a3.7 3.7 0 0 1-1.38-.9 3.7 3.7 0 0 1-.9-1.38c-.16-.42-.36-1.06-.41-2.23-.06-1.27-.07-1.65-.07-4.85s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16zM12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.3-1.46.72-2.12 1.38C1.35 2.68.93 3.35.63 4.14.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.3.79.72 1.46 1.38 2.12.66.66 1.33 1.08 2.12 1.38.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56a5.86 5.86 0 0 0 2.12-1.38c.66-.66 1.08-1.33 1.38-2.12.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91a5.86 5.86 0 0 0-1.38-2.12A5.86 5.86 0 0 0 19.86.63c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0zm0 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32zm0 10.16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm7.85-10.41a1.44 1.44 0 1 1-2.88 0 1.44 1.44 0 0 1 2.88 0z" />
    </svg>
  )
}

function YoutubeIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M23.5 6.2a3.02 3.02 0 0 0-2.12-2.14C19.5 3.55 12 3.55 12 3.55s-7.5 0-9.38.51A3.02 3.02 0 0 0 .5 6.2C0 8.07 0 12 0 12s0 3.93.5 5.8a3.02 3.02 0 0 0 2.12 2.14c1.88.51 9.38.51 9.38.51s7.5 0 9.38-.51a3.02 3.02 0 0 0 2.12-2.14C24 15.93 24 12 24 12s0-3.93-.5-5.8zM9.6 15.57V8.43L15.82 12 9.6 15.57z" />
    </svg>
  )
}

const sections = [
  {
    title: 'Company',
    links: ['About Us', 'Services', 'Projects', 'Industries', 'Sustainability', 'Contact'],
  },
  {
    title: 'Services',
    links: ['Green Cold Storage', 'HVAC Solutions', 'Thermal Energy Storage', 'Solar EPC'],
  },
]

const socials = [
  { icon: LinkedinIcon, label: 'LinkedIn', href: 'https://www.linkedin.com/in/unexgen-technology-24539739a/' },
  { icon: FacebookIcon, label: 'Facebook', href: 'https://www.facebook.com/61584401981736/about/' },
  { icon: InstagramIcon, label: 'Instagram', href: 'https://www.instagram.com/u.nexgen.technology/' },
  // { icon: YoutubeIcon, label: 'YouTube', href: '#' },
]

export function SiteFooter() {
  return (
    <footer className="bg-brand text-primary-foreground">
      <AnimateIn animation="fade-up" className="mx-auto max-w-7xl px-4 py-16 md:px-8">
        <div className="grid gap-10 lg:grid-cols-4">
          {/* Brand Column */}
          <div className="lg:col-span-2">
            {/* Logo */}
            <Link href="#home" className="inline-flex items-center gap-3" aria-label="Unexgen Technology">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary p-1.5 shadow-sm">
                <Image
                  src={logo}
                  alt="Unexgen Technology Logo"
                  width={32}
                  height={32}
                  className="h-full w-full object-contain brightness-0 invert"
                />
              </div>
              <span className="flex flex-col leading-none">
                <span className="font-heading text-lg font-bold">Unexgen</span>
                <span className="text-[10px] font-medium uppercase tracking-[0.18em] text-primary">
                  Technology
                </span>
              </span>
            </Link>

            <p className="mt-5 max-w-sm leading-relaxed text-primary-foreground/70">
              Powering Sustainability. Delivering Innovation. Creating Value.
            </p>
            <p className="mt-3 text-sm text-primary-foreground/55">
              Unexgen Technology Pvt. Ltd. — Green cooling and renewable energy
              engineering for a sustainable future.
            </p>

            {/* Contact Details */}
            <div className="mt-6 space-y-2 text-sm text-primary-foreground/65">
              <p>Chikalthana MIDC, Naregaon, Chhatrapati Sambhajinagar (Aurangabad), Maharashtra 431001</p>
              <p>
                <a href="tel:+91 8275582562" className="transition-colors hover:text-primary">
                  +91 79727 45051
                </a>
                {' · WhatsApp '}
                <a href="https://wa.me/918275582562" className="transition-colors hover:text-primary">
                  +91 82755 82562
                </a>
              </p>
              <p>
                <a
                  href="mailto:office.unexgentechnology@gmail.com"
                  className="break-all transition-colors hover:text-primary"
                >
                  office.unexgentechnology@gmail.com
                </a>
              </p>
            </div>

            {/* Social Icons */}
            <div className="mt-6 flex gap-3">
              {socials.map(({ icon: Icon, label, href }) => (
                <a
                  key={label}
                  href={href}
                  aria-label={label}
                  className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary-foreground/8 transition-colors hover:bg-primary hover:text-white"
                >
                  <Icon className="h-5 w-5" />
                </a>
              ))}
            </div>
          </div>

          {/* Link Columns */}
          {sections.map((section) => (
            <div key={section.title}>
              <h3 className="font-heading text-sm font-bold uppercase tracking-widest text-primary">
                {section.title}
              </h3>
              <ul className="mt-4 space-y-2.5">
                {section.links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      target='_blank'
                      className="text-sm text-primary-foreground/65 transition-colors hover:text-primary"
                    >
                      {link}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom Bar */}
        <div className="mt-12 border-t border-primary-foreground/10 pt-6 text-center text-sm text-primary-foreground/50">
          &copy; {new Date().getFullYear()} Unexgen Technology Pvt. Ltd. All rights reserved.
        </div>
      </AnimateIn>
    </footer>
  )
}
